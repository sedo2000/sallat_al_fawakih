...
async function load(){
  try {
    state.data=await api('/api/store');
    render();
    try{state.user=(await api('/api/auth/me')).user}catch{}
  } catch(e) {
    $('#banners').innerHTML=`<article class="banner"><div class="banner-content"><h2>سلة الفاكهة</h2><p>${esc(e.message||'تعذر تحميل المتجر')}</p></div></article>`;
    $('#dots').innerHTML='';
    toast(e.message||'تعذر تحميل المتجر');
  }
}
function render(){
...
async function saveBanner(){try{let image_url=null;if($('#bi').files[0]){const fd=new FormData();fd.append('image',$('#bi').files[0]);const r=await fetch('/api/admin/upload',{method:'POST',body:fd,credentials:'include'});const d=await r.json();if(!r.ok)throw Error(d.error);image_url=d.url}await api('/api/admin/banners',{method:'POST',body:JSON.stringify({title_ar:$('#bt').value,subtitle_ar:$('#bs').value,image_url,sort_order:0})});toast('تم حفظ العرض');admin()}catch(e){toast(e.message)}}
...
$('#allCats').onclick=()=>{renderProducts(state.data.products);scrollTo({top:document.querySelector('#products').offsetTop-100,behavior:'smooth'})};
document.querySelector('[data-section=featured]').onclick=()=>{const a=state.data.products.filter(p=>p.featured);renderProducts(a);scrollTo({top:document.querySelector('#products').offsetTop-100,behavior:'smooth'})};
document.querySelector('[data-section=all]').onclick=()=>{renderProducts(state.data.products);scrollTo({top:document.querySelector('#products').offsetTop-100,behavior:'smooth'})};
